...
book tracker. See your current read front and center every time you open a
new tab, manage a reading queue, and capture notes and reflections.

* Overview

BookTab is a React single-page application packaged as a Firefox extension
(Manifest V3). There are no background scripts, no content scripts, no
popup -- just a React app running in a privileged new tab page with access
to ~browser.storage.local~ for persistence.
...
All significant decisions are recorded as ADRs in =docs/decisions/=. Key
decisions:
...
See [[file:docs/decisions/README.org][docs/decisions/README.org]] for the
full log.
