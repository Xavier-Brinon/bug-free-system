...
|--------+------------------------------------+---------------+---------|
| 0001   | Vitest as test framework           | 1             | Written |
...
- *Last action* :: [2026-02-26 Thu] -- Phase 1 executed: all 3 plans complete
  (01-01, 01-02, 01-03). ADR infrastructure created, Vitest + React + TanStack
  configured, 5 ADRs written (0001--0005). =npm run test= passes on empty suite.
- *Next action* :: Plan and execute Phase 2 -- Extension Scaffold. Start with
  ADR-0006 (Playwright), ADR-0007 (XState), ADR-0008 (storage). Then scaffold
  the Firefox extension with =vite-plugin-web-extension=.
- *Context for next session* :: Phase 1 is complete -- all tooling and practice
  docs are in place. Phase 2 introduces the actual extension scaffold: manifest,
  Vite build, XState machines, storage layer, Storybook setup, and Playwright
  smoke tests. Three ADRs (0006--0008) must be written before their respective
  implementations. The tidy-up step should be minimal since no feature code
  exists yet.
