...
- [ ] *Phase 1: Tooling & Practice Foundation* -- ADR infrastructure, Vitest
  + TDD methodology locked in, developer practice standards established before
  any feature code
- [ ] *Phase 2: Extension Scaffold* -- Working Firefox extension shell with
  versioned data schema, XState wiring, and all critical pitfalls pre-empted
...
builds on a known-good foundation.
...
1. =docs/decisions/= directory exists with MADR 4.0 template (org-mode) and
   at least two ADRs: one for the test framework choice, one for TDD
   methodology
2. =vitest.config.ts= is present and ~npm run test~ executes successfully,
   returning no failures on an empty test suite
...
- [ ] 01-02-PLAN.md — Vitest configuration (package.json + vitest.config.ts + tsconfig.json)
- [ ] 01-03-PLAN.md — ADR-0001 (Vitest) + ADR-0002 (TDD methodology)
...
via XState v5 actors, and pre-empts every critical pitfall identified in
research.
...
#+CAPTION: Decisions to be documented as ADRs
| Decision                                    | ADR file          | Phase |
|---------------------------------------------+-------------------+-------|
| Test framework: Vitest                      | =0001-vitest.org= |     1 |
| TDD: red-green-refactor vertical slices     | =0002-tdd.org=    |     1 |
| State management: XState v5                 | =0003-xstate.org= |     2 |
| Storage: ~browser.storage.local~ only       | =0004-storage.org= |    2 |
| Book metadata: Open Library (no API key)   | =0005-open-library.org= | 2 |
| Data portability: versioned JSON export     | =0006-json-schema.org= | 6 |
