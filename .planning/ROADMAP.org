...
- *Revised* :: [2026-02-26 Thu] -- Phase 1 gains 3 ADR tasks (React, TanStack
  Query, Storybook); Phase 2 gains Playwright ADR; ADR numbering revised
- *Depth* :: Comprehensive
...
builds on a known-good foundation. This includes ADRs for all stack
decisions made before Phase 2 (React 19, TanStack Query, Storybook).
...
1. =docs/decisions/= directory exists with MADR 4.0 template (org-mode) and
   five ADRs: Vitest, TDD methodology, React 19, TanStack Query, Storybook
...
- ADR-0007 (XState v5 state management)
- ADR-0008 (browser.storage.local only)
- ADR-0009 (Open Library API)
...
#+CAPTION: Decisions to be documented as ADRs (revised numbering)
...
| Book metadata: Open Library (no API key)    | =0009-open-library.org=   |     2 |
...
/Revised: [2026-02-26 Thu] -- Phase 1 updated for React/TanStack/Storybook ADRs; ADR numbering revised 0001-0010/
